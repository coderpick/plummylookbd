                        ...
                        <table class="table table-hover table-bordered" id="sampleTable">
                            ...
                            <tr>
                                ...
                                <td>{{ ucfirst($category->name) }}</td>
                                    <td width="15%"><img class="img-fluid" src="{{ asset($category->icon) }}" alt=""></td>
                                    <td>
                                        {{ ucfirst($category->user->name) }}
                                    </td>

                                    <td>
                                        <form action="{{ route('category.accept',$category->id) }}" method="post" style="display: inline">
                                            @csrf
                                        ...
                                        <form action="{{ route('category.remove',$category->id) }}" method="post" style="display: inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure to remove?')"><i class="fa fa-times"></i> Remove</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
