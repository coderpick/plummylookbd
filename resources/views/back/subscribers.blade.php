                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="example" role="grid">
                            ...
                            @foreach($subscribers as $subscriber)
                                <tr>
                                    <td style="display: none;">{{ $subscriber->id }}</td>
                                    <td>{{ $subscriber->email }}</td>
                                    <td>
                                        <form action="{{ route('subscriber.destroy',$subscriber->id) }}" method="post" style="display: inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Confirm to permanently remove?')">Remove</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Newsletter to Subscribers</h4>
