...
@section('content')
    <div class="app-title">
        ...
        <ul class="app-breadcrumb breadcrumb side">
            ...
            <li class="breadcrumb-item active"><a href="{{ route('sub-category.index') }}">{{ $title }}</a></li>
        </ul>
    </div>
        ...
        <div class="col-md-12">
            <div class="tile">
                <div class="col-md-12 row">
                    <a href="{{ route('sub-category.create') }}" class="btn btn-primary">Add New {{ $title }}</a>
                </div>
                <br>
                <div class="tile-body">
                                ...
                                <td>{{ ucfirst($sub_category->name) }}</td>
                                    ...
                                    <td>
                                        @if($sub_category->deleted_at == null)
                                            ...
                                            <form action="{{ route('sub-category.destroy',$sub_category->slug) }}" method="post" style="display: inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <form action="{{ route('sub-category.restore',$sub_category->id) }}" method="post" style="display: inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Do you want this back?')"><i class="fa fa-undo"></i></button>
                                            </form>

                                            <form action="{{ route('sub-category.delete',$sub_category->id) }}" method="post" style="display: inline">
                                                @csrf
                                                @method('DELETE')
